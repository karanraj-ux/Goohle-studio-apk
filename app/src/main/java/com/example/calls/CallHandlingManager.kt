...
object CallHandlingManager {
    ...
    fun handleIncomingCall(context: Context, number: String) {
        ...
        val ghostMode = runBlocking { settingsRepo.getBooleanSync(SettingsRepository.GHOST_MODE, false) }
        ...
        if (autoForward && !isVip) {
            ...
            // Send Auto-Reply
            try {
                val smsManager = if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) context.getSystemService(android.telephony.SmsManager::class.java) else @Suppress("DEPRECATION") android.telephony.SmsManager.getDefault()
                smsManager.sendTextMessage(number, null, "I'm currently busy, please text or hold on, forwarding your call...", null, null)
            } catch (e: Exception) {
    ...
    fun handleMissedCall(context: Context, number: String) {
        ...
        if (autoReplyEnabled) {
            ...
            if (shouldReply) {
                ...
                try {
                    val smsManager = if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) context.getSystemService(android.telephony.SmsManager::class.java) else @Suppress("DEPRECATION") android.telephony.SmsManager.getDefault()
                    smsManager.sendTextMessage(number, null, autoReplyMessage, null, null)
